...
def test_calculate_final_score_failed_criteria_forces_approval_and_placeholders():
    """Failed criteria (grade_*_error) should force approval and appear in grade_details."""
    state = {
        "rubric": {
            "name": "Test",
            "criteria": [
                {"name": "Clarity", "slug": "clarity", "max_score": 25, "description": "d"},
                {"name": "Content", "slug": "content", "max_score": 35, "description": "d"},
            ],
        },
        "grader_output_keys": ["grade_clarity", "grade_content"],
        "grade_clarity": {
    ...
    assert "failed" in (result.get("approval_reason") or "").lower()
    assert result["max_possible"] == 60
    assert len(result["grade_details"]) == 2

    content_row = next((r for r in result["grade_details"] if r["criterion"] == "Content"), None)
    assert content_row is not None
    assert content_row["score"] == 0.0
    assert content_row["max_score"] == 35


def test_calculate_final_score_missing_grade_data_forces_approval_with_rubric():
    """Missing grade data should force approval and still compute rubric-aligned totals."""
    state = {
        "rubric": {
            "name": "Test",
            "criteria": [
